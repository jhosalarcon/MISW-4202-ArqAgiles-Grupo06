...
class SesionListResource(Resource):
    ...
    @jwt_required()
    def post(self):
        has_acccess_request = requests.get(f"https://acl-queries:5000/api-queries/acl/sesiones/notification_queue", verify=False, headers=request.headers)
        ...
        persona_response = requests.get(f"https://personas-queries:5000/api-queries/personas/{request.json['persona_id']}", verify=False, headers=request.headers)
        actividad_response = requests.get(f"https://actividades-queries:5000/api-queries/actividades/{request.json['actividad_id']}", verify=False, headers=request.headers)
